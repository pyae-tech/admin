$('title').html(get_current_organization_title() + " Invoices");
...
function Load_List() {
    ...
    Pace.start();
    $.ajax({
        url: baseUrl() + "WebServices/WebService_SaleVouncher.asmx/GetAllInvoicesJSON",
        data: "{ " +
            "'search_text':'" + $("#tb_search_text").val() + "' " +
            ",'from_date':'" + $("#tb_search_from_date").val() + "' " +
            ",'to_date':'" + $("#tb_search_to_date").val() + "' " +
    ...
    Columns = [
        ...
        },
        {
            ...
            cellTemplate: function (cellElement, cellInfo) {
                ...
                if (cellInfo.data.InvoiceStatus === "New") {
                    $('<span class="label label-purple full-width">').text(cellInfo.data.InvoiceStatus).appendTo(cellElement);
                }
                else if (cellInfo.data.InvoiceStatus === "Completed") {
                    $('<span  class="label label-primary full-width">').text(cellInfo.data.InvoiceStatus).appendTo(cellElement);
                }
                else if (cellInfo.data.InvoiceStatus === "Cancelled") {
                    $('<span class="label label-danger full-width">').text(cellInfo.data.InvoiceStatus).appendTo(cellElement);
                }
                else if (cellInfo.data.InvoiceStatus === "Order") {
                    $('<span class="label label-warning full-width">').text(cellInfo.data.InvoiceStatus).appendTo(cellElement);
                }
            }
        },
        
        {
            dataField: "VouncherPayAmount",
            caption: "Amount",
            format: {
                type: "fixedPoint",
                precision: 0
...
function BindTable(data) {
    ...
    var dataGrid = $("#gridContainer").dxDataGrid({
        ...
        //        e.component.option('height', '300px');
            ...
            totalItems: [{
                ...
                //showInGroupFooter: true,
                 customizeText: function (data) {
                   // return $('.list_total_amount').html(" : " + NumberWithCommas(data.value) + " MMK");
        ...
        onSelectionChanged: function (selectedItems) {
            ...
            if (data) {
                GoInvoiceList(data.SellVouncherID);
            }
        }
    }).dxDataGrid("instance");
...
function GoInvoiceList(id) {
    GotoPage('Portal/invoice?id=' + id);
}
function generate_list_Total(records) {
    ...
    //    total_amount = total_amount + result[key]['VouncherPayAmount'];
    //});
