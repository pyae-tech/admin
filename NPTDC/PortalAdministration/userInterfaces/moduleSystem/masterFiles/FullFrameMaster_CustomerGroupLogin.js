...
if ($.cookie('CustGroupUserid') == null || $.cookie('CustGroupUserid') == "") {
    GotoPage("portal/customergrouplogin");
}
else {
    $('.lbl_shopname').html($.cookie('StoreName'));
    $('.username').html($.cookie('GroupName'));
    $('.useremail').html($.cookie('orgName'));
    $('.usertype').html($.cookie('reftype'));
    $('.orgname').html($.cookie('orgName'));
    $('.user').html($.cookie('GroupName'));

}

function get_current_user_id() {
    return $.cookie('CustGroupUserid');
}
...
function get_current_AllowedMenus() {
    return $.cookie('AllowedMenu');
} 
...
function get_current_CustomerGroupIDs() {
    return $.cookie('CustomerGroupIDs');
}
...
function get_current_Created_User_id() {
    return $.cookie('createdUserid');
}

function get_current_User_Seq() {
    return $.cookie('Seq');
}



function get_current_CusGp_organization_title() {
    return $.cookie('GroupName') + " | ";
}
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.d.toString().split('~')[0] == "Success") {
                $.cookie('GroupName', "", { expires: 1, path: '/' });
                $.cookie('CustomerGroupLoginID', "", { expires: 1, path: '/' });
                $.cookie('CustomerGroupID', "", { expires: 1, path: '/' });
                $.cookie('orgName', "", { expires: 1, path: '/' });
                $.cookie('orgID', "", { expires: 1, path: '/' });
                $.cookie('CustGroupUserid', "", { expires: 1, path: '/' });
                $.cookie('CustGroupUsercode', "", { expires: 1, path: '/' });
                $.cookie('createdUserid', "", { expires: 1, path: '/' });
                $.cookie('CustomerGroupIDs', "", { expires: 1, path: '/' });
                $.cookie('Seq', "", { expires: 1, path: '/' });
                $.cookie('AllowedMenu', "", { expires: 1, path: '/' });
...
function myFunction() {
//    $.cookie('RefleshEvent', true, { expires: 1, path: '/' });
