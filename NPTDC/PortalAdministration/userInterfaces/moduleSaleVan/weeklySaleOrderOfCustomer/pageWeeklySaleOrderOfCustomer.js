...
function generate_list(records) {
        ...
        if (rowindex == 1) {//paginatin function
            ...
            if (paginationInfos.length > 5) {
                if (paginationInfos[5] != "") {
                    ...
                    $("#tb_customer").val(paginationInfos[10]);
                }
    ...
    $('.checksequence').blur(function () {
        ...
        var error = "";
        if (!equal) {
            error += "Order Qty is not correct";
        }
        if (lessMOQ) {
            error += "Order Qty is not correct.It must greater than MOQ.";
        }
...
function CheckSequence() {
    $('input[id^="tb_edit"]').each(function () {
        ...
        return equal;
        //$(this).attr('sequence_value').includes($(this).val());
        //if ($.isNumeric(oldValue) &&
        //    $(this).val() != oldValue) {
        //    //save
        //    add_order_item($("#tb_oritemid_" + itemNO).val(), itemID, itemNO);
