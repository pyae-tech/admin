$('title').html("Sale Order Detail");

$('#menu_salevan').addClass('active-sub');
$('#menu_salevan_group').addClass('in');
$('#menu_customerorder').addClass('active-link');
...
function LoadNew() {
    ...
    $('#dtp_order_date').periodpicker('change');
    $("#ddl_status").val("New");
    $("#hf_customer_id").val(get_current_Customer_id());
    $("#tb_customer_name").val(get_current_Customer_Name);
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.d != null) {
                ...
                $("#tb_customer_name").val(data.d["CustomerNameEng"]);
    ...
    if (current_invoice_id == '') {
        ...
        $.each(arr_invoice_item, function (key, val) {
            if (arr_invoice_item[key]['id'] == current_invoice_id) {
                ...
                arr_invoice_item[key]['item_name'] = $('#tb_item').val();
...
function Load_Customer_List() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.d != null) {
                generate__customer_list(data.d);
            }
        },
        error: function (xhr, msg) {
            LogJSError('Web Service Fail: ' + msg + '\n' + xhr.responseText);
